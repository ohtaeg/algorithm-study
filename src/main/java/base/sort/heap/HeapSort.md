...
- `Heap` 자료구조를 이용하여, `Heapify`연산을 통해 정렬하는 방법.
    1. 주어진 1차원 배열을 힙으로 만든다.
        - 마지막 리프 레벨의 마지막 노드의 부모 노드부터 `heapify` 과정을 거친다.
              즉 1..n개가 있을때, 2/n부터 루트 노드 까지 `heapify` 과정을 거친다.
    2. 힙에서 최대값 Root와 가장 마지막 값과 바꾼다.
    3. 힙의 크기가 1개 줄어든것으로 간주한다. (마지막값은 최대값이고, 마지막을 제외해도 heap이기 때문)
    4. 힙의 크기가 1보다 크면 다시 `heapify` 과정을 통해 2~3번 과정을 반복한다.
...
### 힙 정렬의 시간 복잡도
- heapify를 2/n만큼 반복하며 트리 높이에 비례하므로 시간 복잡도 : O(N * logN)
